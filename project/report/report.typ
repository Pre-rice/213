...
本研究的特征工程与模型设计均以#hei[市场微观结构理论]（Market Microstructure Theory）@microstructure 为经济学支撑。以下按特征类别，阐述各类信号的市场行为依据。

=== 1.2.1　订单流失衡信号：价格冲击的直接度量（OVI / TI 脉冲类）
...
=== 1.2.2　委托簿深度与逆向选择：耐心资金的方向信号（OBI / 大单 / 价差类）
...
=== 1.2.4　日内时间效应与价格惯性：行为周期与均值回复（时间 / 滞后收益类）
