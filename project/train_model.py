...
ENSEMBLE_EWMA_BETA   = 0.02 # IC 的 EWMA 平滑系数（β小=慢更新=更稳定，窗口边界不敏感）
...
def dynamic_ensemble(y_test, model_preds,
    ...
    if update_freq > 1:
        ...
        frozen_weights = np.zeros_like(weights)
        frozen_weights[update_indices] = weights[update_indices]
        # 前向填充：非更新点复用上次权重
        w_df = pd.DataFrame(frozen_weights)
        for j in range(n_models):
            col = w_df.iloc[:, j].copy()
            col[col == 0.0] = np.nan
            col.iloc[:warmup] = weights[:warmup, j]
            w_df.iloc[:, j] = col.ffill()
