...
def process_day_data(day_data):
    ...
    ── 板块均值信号 ─────────────────────────────────────────────────────────────
     18.  Sect_OBI1           - 板块 (ABCD) 一档委托失衡均值
     19.  E_TI_rel_600        - E 的长期 TI 相对板块偏差 (均值回复)
     20.  Sect_TI_p40         - 板块成交量失衡 SMA40 脉冲
     21.  Sect_OVI_p20        - 板块委托量失衡 SMA20 脉冲
     22.  Sect_ONI_p30        - 板块委托笔数失衡 SMA30 脉冲
     23.  Sect_OVI_ep5        - 板块委托量失衡 EMA5  - EMA600 (超短期板块动能)
     24.  Sect_OVI_ep15       - 板块委托量失衡 EMA15 - EMA600

    ── 日内时间特征 ─────────────────────────────────────────────────────────────
     25.  aft_13800           - tick_index > 13800 二值 (交易日后半段指示)
     26.  aft_12000           - tick_index > 12000 二值 (稍早分界，与 25 互补)

    ── 滞后已实现收益特征 ─────────────────────────────────────────────────────
     27.  sect_ret_lag        - 板块(ABCD)过去5分钟平均已实现收益（动量/反转）
     28.  e_ret_lag           - E 过去5分钟已实现收益（均值回复信号）
     29.  past_ret_30         - E 中间价过去 15 秒收益率 (30 ticks)
     30.  past_ret_60         - E 中间价过去 30 秒收益率 (60 ticks)
     31.  past_ret_120        - E 中间价过去 1 分钟收益率 (120 ticks)
     32.  past_ret_300        - E 中间价过去 2.5 分钟收益率 (300 ticks)
     33.  past_ret_600        - E 中间价过去 5 分钟收益率 (600 ticks)
     34.  past_ret_900        - E 中间价过去 7.5 分钟收益率 (900 ticks)
                                Day5 IC=+0.26（均值回复），Day3 IC=+0.20，Day1 IC=+0.17

    ── 板块短期价格收益率 & 横截面相对收益（新增）────────────────────────────
     35.  sect_mid_ret_30     - 板块(ABCD)平均中间价过去 15 秒收益率 (订单流外的价格信息)
     36.  sect_mid_ret_120    - 板块(ABCD)平均中间价过去 1 分钟收益率
     37.  csm_ret_120         - sect_mid_ret_120 - past_ret_120（E 相对板块 1 分钟落后量）
     38.  e_spread_pulse      - E 相对买卖价差偏离 600-tick 均值（做市商不确定性信号）
     39.  lot_imb_15          - 15-tick 平均买入笔金额 vs 卖出笔金额相对偏差（大单方向）
                                在全部 5 日 IC 符号一致为正，弱信号日（1,5）尤为突出
     40.  sect_lot_imb_15     - 板块(ABCD)平均大单成交失衡（机构活跃度方向，全日一致正向）

    ── 深层委托簿失衡脉冲（新增）──────────────────────────────────────────────
     41.  obi_deep_p15        - E 股 2-5 档委托失衡 SMA15 - SMA600 脉冲
                                与 OVI（1 档）正交（相关性 0.23-0.26），捕捉深层买卖双方
                                "耐心资金"的相对强弱，在弱信号日（1/5）有额外正向贡献。

    ── 累计成交流量失衡（新增）──────────────────────────────────────────────────
     42.  cum_flow_imb        - E 股日内累计买卖成交量失衡，对 600-tick 均值去趋势
                                (cum_buy - cum_sell) / cum_total - SMA600 偏差
                                在 Day5 有 IC=+0.23（TI 偏相关 IC=+0.24），捕捉日内趋势方向。
     43.  sect_cum_flow_imb   - 板块(ABCD)平均累计成交量失衡去趋势
                                与 cum_flow_imb 正交（Day3 IC=+0.26，Day5 偏相关 IC=+0.14）
     44.  e_ret_lag2          - E 股 Return5min 900-tick 滞后（7.5 分钟前已实现收益，无前视）
                                Day5 IC=-0.25（偏相关 -0.18），Day2/3 IC=-0.18/-0.18

    ── 交互特征（新增）─────────────────────────────────────────────────────────
     45.  ovi_x_abs_ret       - OVI_p15 × |past_ret_600|（幅度条件化 OVI 信号）
                                OVI 信号在有较大先期价格波动时更可靠；全 5 日 IC 一致正向（≈0.13）
     46.  tbv_x_ovi           - TotalBidVol（相对均值）× OVI_p15（深度条件化 OVI 信号）
                                买方深度越深，OVI 信号越可靠；全 5 日 IC 一致正向（≈0.12）
     47.  srl_x_ovi           - sect_ret_lag × OVI_p15（板块滞后收益 × E 委托动能）
                                板块先期涨跌方向与 E 当前委托方向的共同确认信号（≈0.10）
     48.  ret_x_cum           - past_ret_600 × cum_flow_imb（价格反转 × 日内流量趋势）
                                Day4 IC=+0.25，捕捉"流量积累方向与近期价格反转"的双重确认
     49.  oni_x_ovi           - ONI_p15 × OVI_p15（委托笔数 × 委托量双重失衡共振）
                                当订单数量与资金量方向一致时信号更强（全 5 日一致正向）

    ── Iter14 新增（时间段分析指导）──────────────────────────────────────────────
     50.  book_pres_pulse     - E 股全 5 档委托失衡 SMA15 - SMA600 脉冲
                                与 obi_deep_p15（2-5档）互补（含1档即时深度），
                                全 5 日 IC 一致正向（0.051-0.116），均值约 0.079
     51.  ret_x_ti600         - past_ret_600 × TradeImb_600（价格方向 × 成交流量方向确认）
