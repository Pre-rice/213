...
213/
├── Example/    # 给定的项目简单示例
├── project/    # 项目工程文件夹，包含完整的工程文件
├── report/     # 项目论文文件夹
├── submit/     # 成品提交文件夹，包含成品工程文件及文字报告
├── test/       # 测试工程文件夹
├── 比赛数据/    # 给定的五只股票的五日原始数据
├── .gitignore   # Git忽略文件
└── README.md    # 本说明文档，包含项目说明与比赛题目说明
```

推荐阅读方法：先阅读下文的比赛题目说明，再阅读`submit`文件夹中的`文字报告.pdf`，可以查看`代码附件`，按照其中`README.md`的说明安装依赖库测试运行`main.py`。如需进一步了解工程细节，可以具体查看`project`文件夹。如需查看论文的源代码，可以具体查看`report`文件夹。其余文件夹仅供参考。
