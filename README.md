# 比赛题目说明
### 股票未来收益率预测
板块效应指的是股票市场中同一板块的个股因共同特点或关联题材，呈现协同涨跌的联动现象。因而我们可以利用板块效应来分析股票的未来收益率。
##### 比赛目标  
现有A,B,C,D,E五只股票，他们属于相同风格概念板块，会呈现板块联动效应。你将利用股票E的自身数据信息和板块内其他股票数据信息构建模型来预测股票E的未来5分钟收益率(Return5min)。你的模型将根据测试集数据最终进行评估。
##### 评估指标
股票E的未来5分钟预测收益率，与真实收益率的N日平均IC值。排名按测试集平均IC值从大到小排名，平均IC值高则排名高。

注：IC指皮尔森相关系数，就是高中学的样本相关系数r。
...
|------|------|------|
| **快照类数据** | | **当前时刻的供需数据(存量)** |
| Time | int64 | 时间(HHMMSSmmm) | 93000000(9:30:00.000)至112959500(11:29:59.500)、130000000(13:00:00.000)至145000000(14:50:00.000)
...
注：Return5min的具体公式暂不清楚。但是记MidPrice为BidPrice1与AskPrice1的均值，以MidPrice五分钟后的相对增幅计算Return5min，得到的数据与给定数据的误差不超过1%。

---

# 作品提交格式要求
本次大赛的作品应分为两部分：文字报告部分和代码附件部分，均以电子版提交。其中，文字报告部分阐述建模的过程以及展示模型的运行结果。代码部分以附件的形式提交，默认编程语言为python。我们将在全部作品提交后，根据金融市场在未来发生的实际数据，并运行参赛者提交的代码，得到所有有效作品在真实市场中的“样本外”预测效果，并据此为参赛作品评分。因而务必保证所提交的代码能够按照我们所要求的输入输出格式正确运行。
...
注：推荐使用Typst排版，能够方便地书写公式、代码、管理参考文献等
...
├── utils.py         # [必须提交] 工具函数文件(自定义工具函数，格式无要求，建议拆分为多文件)
...
#### 注释与命名规范
 ...
 - 代码需符合语言标准，变量名采用下划线命名法(如 train_data )。
 - 关键步骤添加注释说明，例如：
    - 数据预处理：去除缺失值(作者：李四)
...
```
注：pip 是 Python 的 包管理工具，用于安装和管理 Python 第三方库（包）。为了在一个电脑中管理多个不同的 Python 环境，需要下载第三方环境管理器工具，创建多个虚拟环境。不过 PyCharm 这样的集成开发环境也可以便捷地创建虚拟环境。
...
#### 建议
- 参考现成的赛题、工程、论文等，模仿学习
- AI是指导老师：我们对相关知识完全不了解，需要靠AI提供信息指明方向，告诉我们需要做什么，才能进行具体的实践
- 使用git远程协作，各个开发环境都可使用
- 如何分工协作？建议先所有人先共同完成一个小样本测试，有了解了后续再明确分工
...
**核心职责**：
1. **模型架构设计**
   - 选择合适的模型（LightGBM、LSTM、Transformer等）
   - 设计模型集成策略
   - 超参数优化方案
